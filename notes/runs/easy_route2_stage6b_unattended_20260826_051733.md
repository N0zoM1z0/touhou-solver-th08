...
The paired run halves hits without any global query, so Easy does not yet justify a more complex corridor solver.  Post-run AUD-093 identifies the two positive-clearance/no-warning contacts as existing bullet slots with reversed snapshot alignment, not future births.  Exact replay at frames 11412 and 53730 rejects the issued actions under the bullet pool's full age support while alternatives remain.  The generic correction passes offline tests and is the next focused Practice falsifier.  Only after that result should the remaining modeled losing-prefix witnesses drive a cheap local viable-continuation experiment.  Preserve the global framework for authoritative harder-mode roots; do not add a stage, spell, or difficulty branch.
